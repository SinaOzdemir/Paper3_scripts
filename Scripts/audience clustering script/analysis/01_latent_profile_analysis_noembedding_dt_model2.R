...
data<- readRDS(file = paste0(data_folder,"/clustering_df_scaled.RDS")) %>% select(starts_with("X"))

test<-tidyLPA::estimate_profiles(df = data[1:210,],n_profiles = 1:3,variances = "equal",covariances = "zero")

test_estimates<-test %>% get_estimates()
test_fits<-test %>% get_fit()
...
for (i in 1:1000) {
  ...
  lpa_results[[i]]<- lpa_estimator(data = sample_data,
                                   cluster_count = 1:113,
                                   model = 2) #equal variance and covariances both fixed to 0
  saveRDS(lpa_results,file = paste0(results_folde,"/scaled_data_lpa_model2.RDS"))
