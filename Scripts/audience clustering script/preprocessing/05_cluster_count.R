...
clustering_data = readRDS(file = paste0(data_folder,"/clustering_matrix_scaled.RDS"))
...
errored_index<-c()
for (i in 1:length(good_indices)) {
  ...
  if(inherits(possible_error,"error")){
    ...
    saveRDS(optimum_numbers,file = paste0(data_folder,"/optimum_cluster_number.RDS"))  
  }
